...
function toast(text, duration, btn_ok_caption, on_ok, on_timeout)
{
	...
	if (btn_ok_caption.length > 0) {
		html = file2bin(context.libpath() + "html/toast.html");
	}
	else {
		html = file2bin(context.libpath() + "html/toast_centered.html");
	}
	...
	if (btn_ok_caption.length > 0) {
		jQuery("#BTN_OK_" + toast_id).off("click").on("click", function() {
			...
			ripple("BTN_OK_" + toast_id, function() {
				cleanup();
				on_ok();
			});
		});
...
function alert(text, title, btn_ok_caption, on_ok)
{
	...
	var html = file2bin(context.libpath() + "html/alert.html");
...
function confirm(text, title, btn_ok_caption, btn_cancel_caption, on_ok, on_cancel)
{
	...
	var html = file2bin(context.libpath() + "html/confirm.html");
...
function prompt(text, title, suggestions, value, cast, digits, password, btn_ok_caption, btn_cancel_caption)
{
	...
	return new Promise(
		(resolve, reject)=>{
			...
			var previous_keyboard_did_show_callback = application.keyboard_did_show_callback;
			var previous_keyboard_did_hide_callback = application.keyboard_did_hide_callback;
			...
			var hide = function() 
			{
				...
				application.reg_keyboard_did_show_callback(previous_keyboard_did_show_callback);
				application.reg_keyboard_did_hide_callback(previous_keyboard_did_hide_callback);
			...
			{
				console.info("IN prompt()->onbackbutton()");
				failed();
			};

			var keyboard_did_show = function()
			{
				console.info("IN prompt()->keyboard_did_show()");
				var container  = document.getElementById("DIV_CON_"+ prompt_id).getBoundingClientRect();
				//console.log("container=" + JSON.stringify(container));
				//console.log("container.height=" + container.height);
				var element    = document.getElementById("DIV_PROMPT_"+ prompt_id).getBoundingClientRect();
				//console.log("element=" + JSON.stringify(element));
				//console.log("element.height=" + element.height);
				var limitH     = container.height - application.keyboardHeight;
				//console.log(limitH);
				var marginVert = Math.round((limitH - element.height) / 2);
				//console.log(marginVert);
				//jQuery("#DIV_PROMPT_" + prompt_id).css("top", marginVert + "px");
				//jQuery("#DIV_PROMPT_" + prompt_id).show();
				jQuery("#DIV_PROMPT_" + prompt_id).animate( {"top" : marginVert + "px"}, 200 );
			};

			var keyboard_did_hide = function()
			{
				console.info("IN prompt()->keyboard_did_hide()");
				var element = document.getElementById("DIV_PROMPT_"+ prompt_id);
				var h = Math.round((window.innerHeight - element.getBoundingClientRect().height) / 2);
				jQuery("#DIV_PROMPT_" + prompt_id).css("bottom", h + "px");
			};
			...
			var get_result = function()
			{
				...
				if (strlen(String(value)) > 0) {
					...
					application.media(context.libpath()+"assets/mp3/negative.mp3");
			...
			var onshow = function()
			{
				...
				application.reg_back_button_callback(onbackbutton);
				application.reg_keyboard_did_show_callback(keyboard_did_show);
				application.reg_keyboard_did_hide_callback(keyboard_did_hide);
				...
				jQuery("#BTN_DEL_PROMPT_" + prompt_id).off("click").on("click", function(){
					noripple(function(){
						jQuery("#INP_PROMPT_" + prompt_id).val("");
						jQuery("#INP_PROMPT_" + prompt_id).focus();
					});
			...
			var show = function()
			{
				...
				var html = file2bin(context.libpath() + "html/prompt.html");
				//console.log(html);
				...
				if (strcasecmp(cast, "CODE") === 0) {
					delay(100, function(){
						...
						console.log("I AM HERE digits=" + digits);
