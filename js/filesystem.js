...
function getFileShortcutIcon (mimeType, relativeToLocalPath) 
{
	if (typeof relativeToLocalPath === "undefined") { relativeToLocalPath = true; }
	console.info("IN binaries.getFileShortcutIcon() mimeType='" + mimeType + "' relativeToLocalPath=" + relativeToLocalPath);
	
	var result = "img/doc_other_file.png";

	if (strmatch(mimeType, "IMAGE")) {
		result = "img/doc_image.png";
	}
	else if (strmatch(mimeType, "VIDEO")) {
		result = "img/doc_video.png";
	}
	else if (strmatch(mimeType, "AUDIO")) {
		result = "img/doc_audio.png";
	}
	else if (strmatch(mimeType, "application/pdf")) {
		result = "img/doc_adobe_acrobat.png";
	}
	else {
		var wordDocuments = [
		...
		if (wordDocuments.includes(mimeType)) {
			result = "img/doc_ms_word.png";
		}
		else if (spreadsheetsDocuments.includes(mimeType)) {
			result = "img/doc_ms_excel.png";
		}
		else if (presentationDocuments.includes(mimeType)) {
			result = "img/doc_ms_powerpoint.png";
		}
	}

	if (relativeToLocalPath) {
		result = context.libpath() + result;
	}
