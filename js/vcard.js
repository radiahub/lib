...
function vcardSave (vcf_text)
{
	return new Promise(
		(resolve, reject) => {
			...
			if ((context.is_cordova()) && (typeof navigator.contacts !== "undefined")) {
				...
				if (strlen(displayName) > 0) {
					...
					contact.save(
						...
						function() { 
							M.error("Rejected by contact.save()");
					...
					M.error("VCard parse FN error");
					reject();
				}

			}
			else {
				M.error("Not a mobile application or plugin not installed");
				reject();
			}
...
function vcardShare (vcf_text)
{
	return new Promise(
		(resolve, reject) => {
			...
			//console.log(vcf_text);
			if ((context.is_cordova()) && (typeof window.plugins.socialsharing !== "undefined")) {
